...
//            }
            if ($each['total_amount']>0 || $each['has_ratio_products']) {
                $id_payment = $this->addPayment(round($each['total_amount']*$discountFactor,2),$user, $farms[$id_farm],$contract, $each['payment_types'],round($each['split_payments']*$discountFactor,2),$farms[$id_farm]->getCheckPayableTo(),$each['chosen_payment']);
